...
contract NearBridge is INearBridge, AdminControlled {
    ...
    // interface deposit/withdraw and check block challenge
    ...
    // init validators list of the current epoch
   ...
   // bridge state var
    ...
    // fill block BlockProducers and update state
...
# Rainbow bridge vs Horizon Bridge
## Rainbow bridge
- Relayers need to continuously send ethereum and NEAR blocks to smart contracts, which requires a large amount of gas cost.
  
- In order to avoid the continuous increase of states in the NEAR contract, the bridge restricts saving only the block headers synchronized in the last 7 days. If the contract also limits validation to only 7-day logs, the light Client's security will be significantly reduced. Malicious Ethereum Relayer can start creating a fake chain of only seven days worth of headers appended to a valid prefix and present it to The NEAR contract. If the NEAR contract also contains previous seven-day logs from old blocks on the NEAR blockchain, a significant gas cost will be required.
  
- Rainbow's Ethereum side contract does not validate all validator signatures in each NEAR block header.
